...
const DEFAULT_FILTERS: UrlFilterConfig = {
  blocked_host_suffixes: [
    // Add stable suffixes here, or put them in data/blacklist_domains.txt
  ],
  blocked_host_keywords: [
    // Hostname keywords that strongly imply non-evergreen / auth / portals
    'bank',
    'banking',
    'ebank',
    'ebanking',
...
function normalizeUrl(
  ...
  try {
    ...
    // Sort remaining query params for consistent normalization
...
function titleMatchesBlacklist(title: string): boolean {
  ...
  if (
    BLACKLIST_TITLES_PARTIAL.some((blocked) => titleLower.includes(blocked.toLowerCase()))
  )
    return true
...
function getBlockReason(
  ...
  try {
    ...
    if (cfg.blocked_host_suffixes.some((s) => hostMatchesSuffix(host, s)))
      return 'host_suffix'
  } catch {
    return 'invalid_url'
  }
...
interface UrlStats {
  ...
  lastVisit: number
  title: string
  visits: number[] // timestamps in ms
}
...
function processHistoryFiles(dataDir: string): {
  ...
  for (const file of files) {
    ...
    try {
      ...
      for (const item of history) {
        ...
        if (
          !item.url ||
          INTERNAL_URL_PREFIXES.some((prefix) => item.url.startsWith(prefix))
        )
          continue
  ...
  // Sort by frequency then recency for crawler queue
...
async function importToDatabase(urlStats: Map<string, UrlStats>): Promise<void> {
  console.log('\nInserting into database...')

  let urlsInserted = 0
  let visitsInserted = 0

  for (const stats of urlStats.values()) {
    // Upsert URL
    const existingUrl = await db
      .selectFrom('url')
      .select('url_id')
      .where('url_norm', '=', stats.url_norm)
      .executeTakeFirst()
    ...
    if (existingUrl) {
      ...
      await db
        .updateTable('url')
        .set({ last_seen_at: new Date(stats.lastVisit) })
        .where('url_id', '=', existingUrl.url_id)
        .execute()
      ...
      const result = await db
        .insertInto('url')
        .values({
          url_norm: stats.url_norm,
          domain: stats.domain,
          path: stats.path,
          first_seen_at: new Date(stats.firstVisit),
          last_seen_at: new Date(stats.lastVisit),
        })
        .returning('url_id')
    ...
    // Insert visits
    for (const visitTime of stats.visits) {
      await db
        .insertInto('visit')
        .values({
          url_id: urlId,
          visited_at: new Date(visitTime),
        })
        .execute()
      visitsInserted++
    }
  ...
  console.log(`Inserted ${urlsInserted} new URLs`)
  console.log(`Inserted ${visitsInserted} visit events`)
}
...
async function main() {
  ...
  // Process history files and create prioritized queue
  const { urlStats, sorted, report } = processHistoryFiles(dataDir)
  ...
  await importToDatabase(urlStats)
  await db.destroy()
}
